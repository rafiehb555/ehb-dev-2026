      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            >
              {item.icon ? <span className="shrink-0 text-slate-300">{item.icon}</span> : null}
            ...
            {isOpen ? (
              <div className="border-t border-white/5 px-4 py-3 text-[12px] leading-relaxed text-slate-300">{item.content}</div>
            ) : null}
          </div>
        );
      })}
