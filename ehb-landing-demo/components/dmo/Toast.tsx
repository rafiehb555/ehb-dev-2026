...
export function Toast(props: { toast: ToastState; onClose: () => void }) {
  ...
  }, [props.toast.open, props.onClose]);
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1">
                <div className="font-semibold text-white">{props.toast.title}</div>
                {props.toast.message ? <div className="text-[11px] text-slate-200">{props.toast.message}</div> : null}
              </div>
              <button
                type="button"
